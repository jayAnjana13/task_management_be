...
class TaskService {
    ...
    async getTasks(userId, projectId, params) {
        ...
        const { clause: filterClause, params: filterParams, nextParamIndex, } = (0, pagination_1.buildWhereClause)(filters, allowedFilterMapping, paramIndex);
        // Build search clause
        const { clause: searchClause, params: searchParams } = (0, pagination_1.buildSearchClause)(search, ["t.title", "t.description"], nextParamIndex);
    ...
    `;
        const tasksResult = await (0, database_1.query)(tasksQuery, [...allParams, limit, offset]);
        const tasks = tasksResult.rows.map(this.mapTask);
        return { tasks, total };
    }
    // Get task by ID
    async getTaskById(taskId, userId) {
        const result = await (0, database_1.query)(`SELECT t.id, t.title, t.description, t.status, t.priority, 
              t.project_id, t.assignee_id, t.reporter_id, t.due_date,
              t.estimated_hours, t.actual_hours, t.tags, t.position,
              t.created_at, t.updated_at,
              p.id as project_id, p.name as project_name,
       ...
       WHERE t.id = $1`, [taskId]);
        ...
        // Check project access
        const hasAccess = await project_service_1.projectService.userHasAccess(task.project_id, userId);
        if (!hasAccess) {
    ...
    async validateAssignee(projectId, assigneeId) {
        if (!assigneeId)
            return;
        const result = await (0, database_1.query)(`SELECT pm.id FROM project_members pm
       WHERE pm.project_id = $1 AND pm.user_id = $2`, [projectId, assigneeId]);
        if (result.rows.length === 0) {
            throw new errors_1.ForbiddenError("Assignee must be a member of the project");
        }
    }
    // Create new task
    async createTask(userId, data) {
        // Check project access
        const hasAccess = await project_service_1.projectService.userHasAccess(data.projectId, userId);
        if (!hasAccess) {
            throw new errors_1.ForbiddenError("You do not have access to this project");
        }
        // Validate assignee is a project member
        ...
        // Get next position
        const positionResult = await (0, database_1.query)(`SELECT COALESCE(MAX(position), -1) + 1 as next_position 
       FROM tasks WHERE project_id = $1`, [data.projectId]);
        const position = positionResult.rows[0].next_position;
        // Create task
        const result = await (0, database_1.query)(`INSERT INTO tasks (title, description, status, priority, project_id, 
    ...
    async updateTask(taskId, userId, data) {
        ...
        if (data.assigneeId !== undefined) {
            await this.validateAssignee(existingTask.projectId, data.assigneeId || undefined);
        }
        // Build update query
        const updates = [];
        const params = [];
        ...
        params.push(taskId);
        await (0, database_1.query)(`UPDATE tasks SET ${updates.join(", ")} WHERE id = $${paramIndex}`, params);
    ...
    async deleteTask(taskId, userId) {
        ...
        const result = await (0, database_1.query)("DELETE FROM tasks WHERE id = $1 RETURNING id", [
            taskId,
        ]);
        if (result.rows.length === 0) {
            throw new errors_1.NotFoundError("Task not found");
    ...
    async updateTaskPositions(updates, userId) {
        for (const update of updates) {
            ...
            await (0, database_1.query)(`UPDATE tasks SET ${setClause} WHERE id = $${params.length}`, params);
        }
    }
