...
class ProjectService {
  ...
  async getProjects(userId: string, params: QueryParams): Promise<ProjectListResult> {
    ...
    const { clause: filterClause, params: filterParams, nextParamIndex } = 
      buildWhereClause(filters, allowedFilterMapping, 2);
    ...
    `;
    const projectsResult = await query(projectsQuery, [...allParams, limit, offset]);
  ...
  // Get project by ID
  async getProjectById(projectId: string, userId: string, userRole?: 'admin' | 'user'): Promise<Project> {
  ...
  async updateProject(
    ...
    if (!isOwner) {
      throw new ForbiddenError('Only the project owner can update the project');
    }
  ...
  async addMember(
    ...
    const userResult = await query('SELECT id FROM users WHERE id = $1', [data.userId]);
  ...
  async userHasAccess(projectId: string, userId: string, userRole?: 'admin' | 'user'): Promise<boolean> {
  ...
  async getAllProjectsAdmin(params: QueryParams): Promise<ProjectListResult> {
    const { page, limit, offset, sortBy, sortOrder, search, filters } = params;
    ...
    const { clause: filterClause, params: filterParams, nextParamIndex } = 
      buildWhereClause(filters, allowedFilterMapping, 1);
  ...
  private mapProjectAdmin(row: any): Project & { messageCount?: number } {
    return {
      id: row.id,
      name: row.name,
  ...
  private mapProjectMember(row: any): ProjectMember {
    return {
      id: row.id,
